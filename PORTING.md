...
Java sources, the access widener, and Mixin targets still need the code port.
...
- Both loaders fail on old RenderPipeline references. NeoForge also reports direct access to the now-private Entity.invulnerableTime field.
...
Existing unrelated issue: `group=de.keksuccino.drippy` is still inherited from
the old project setup. It was preserved because changing published Maven
coordinates is separate from preparing the Minecraft version.
