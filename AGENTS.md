...
- SpiffyHUD (aka. Spiffy) is a Minecraft Java 1.19.2 mod that uses the MultiLoader layout with shared logic under `common` and loader-specific wrappers under `fabric` and `forge`.
- SpiffyHUD is an addon for "FancyMenu" and needs FancyMenu as mandatory dependency.
...
- Follow existing packages under `de.keksuccino.spiffyhud`, mirroring existing sub-packages like `customization`, `events`, and `platform` to keep cross-loader boundaries clear.
- Name resources with the `spiffyhud` prefix (e.g., `spiffyhud.mixins.json`, `spiffyhud.accesswidener`) so Gradle and the loaders resolve them consistently.
...
- Place shared mixins under `common/src/main/java/de/keksuccino/spiffyhud/mixin/mixins/common/<side>` and mirror the existing folder depth when adding new targets.
...
## Networking & Packets
- FancyMenu uses its own custom packet system and SpiffyHUD should use FancyMenu's packet system.
- If you need to add packets for a feature, make sure to analyze the `de.keksuccino.fancymenu.networking` package first, to understand how packets get implemented and registered.
...
- The `minecraft` directory has subdirectories for the different launchers, so you can compare launcher-specific changes to Minecraft code, such as `fabric`, `forge`, `forge`, etc., and inside each is the Minecraft source code.
