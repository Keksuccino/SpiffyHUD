...
- This project is "SpiffyHUD" (aka. "Spiffy"), which is a Minecraft Java 26.2 mod that uses the MultiLoader layout with shared logic under `common` and loader-specific wrappers under `fabric` and `neoforge`.
- It also has a special `earlywindow` submodule for NeoForge's "Early Loading Screen".
- Spiffy is an addon for "FancyMenu", which is a mod to customize Minecraft's menu screens. Spiffy expands FancyMenu to support the loading overlay that shows up when launching the game and when reloading resources.
...
- Follow existing packages under `de.keksuccino.spiffyhud`, mirroring existing sub-packages to keep cross-loader boundaries clear.
...
- Spiffy has access to Mixin Extras.
...
## Networking & Packets
- FancyMenu uses its own custom packet system, and Spiffy should use that custom packet system when it needs to add packets.
